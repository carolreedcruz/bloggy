  ...
  return (
    <div className="sticky top-0 z-50 bg-black shadow-sm py-4">
      <nav className="flex items-center justify-between px-4">
        ...
        <Link
          className="text-white text-xl font-bold hover:text-yellow-300 absolute left-1/2 transform -translate-x-1/2"
          to="/"
        >
          Home
        </Link>
          ...
          {user && (
            <>
              <span className="text-white font-bold">
                Signed in as: <span className="font-normal">{user.email}</span>
              </span>
              <button
                onClick={() => {
                  signOut(auth);
